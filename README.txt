...
- Each player will take turn to input a number and send it to the server, the number will be added up to the sum. Whoever reaches the generated number first will win the game.
(- Basic GUI built with PythonQt5)

2. Run server: Change directory to the folder contains the server files	
	Command line: python3   server-Easy.py   IP-address
	Server Easy will run on Port 43500
	
	Command line: python3   server-Hard.py   IP-address
	Server Hard will run on Port 43505

3. Run client: Change directory to the folder contains the file (client.py)
	Command line: python3   client.py   IP-address/hostname 
...
4. Click on (X) to quit the server
